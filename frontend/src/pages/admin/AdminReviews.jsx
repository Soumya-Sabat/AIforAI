...
import { FiGrid, FiFileText, FiEye, FiUsers, FiShield, FiActivity, FiSettings, FiMenu, FiSearch, FiLogOut } from "react-icons/fi";
...
export default function AdminReviews() {
  ...
  const updateStatus = async (reportId, decision, remarks) => {
    if (!remarks || remarks.trim().length < 3) { alert("⚠ Feedback must be at least 3 characters."); return; }
    try {
      const res = await fetch(`http://localhost:5000/api/reports/admin-review/${reportId}`, {
        method: "POST",
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-[#0a0a12] text-white">
        ...
        <p className="mt-4 animate-pulse text-purple-300">Fetching review requests…</p>
      </div>
    );
  }
    ...
    <div className="flex min-h-screen bg-[#0a0a12]">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="p-4 sm:p-6 text-white overflow-x-hidden">
          <h1 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6">Review Monitoring</h1>

          {/* Search + Filters — stacked on mobile, grid on sm+ */}
          ...
          <p className="text-xs text-gray-500 mb-4">Showing {paginated.length} of {sorted.length} results</p>
          ...
          {paginated.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
              <AnimatePresence>
                {paginated.map((r) => {
                  const myReview    = r.reviews?.find(rv => rv.reviewType === "admin" && rv.reviewedBy?._id?.toString() === currentUserId);
                  const adminReview = r.reviews?.find(rv => rv.reviewType === "admin");
                  const statusLabel = adminReview?.status || "pending";

                  return (
                    <motion.div key={r._id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                      ...
                      <div className="mt-3 rounded-lg overflow-hidden">
                        <video controls className="w-full h-44 sm:h-52 object-cover rounded-lg"
